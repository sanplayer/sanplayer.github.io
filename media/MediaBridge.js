...
class MediaBridge {
    ...
    static logDebug(message) {
        // Enviar log para Java que aparece no logcat do Android
        if (!this._hasAndroidBridge()) return;
        try {
            if (typeof window.Android.logDebug === 'function') {
                window.Android.logDebug('[JS] ' + message);
            }
        } catch (e) {
            console.warn('[MediaBridge] ⚠️ Android logDebug failed:', e.message);
        }
    }
